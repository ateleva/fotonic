...
export default function VaultLock() {
  ...
  const onSubmit = async ({ password, otp }) => {
    try {
      ...
      setError('root', { type: 'manual', message: err.message || 'Invalid password or code' })
    }
  }
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-md w-full max-w-md p-8">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <FormField label="Vault Password" required htmlFor="password" error={errors.password?.message}>
            <input id="password" type="password" autoComplete="current-password" className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" {...register('password', { required: 'Password is required' })} />
          </FormField>
          <FormField label="Authenticator Code" required htmlFor="otp" error={errors.otp?.message}>
